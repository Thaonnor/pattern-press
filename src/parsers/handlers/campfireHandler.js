...
/**
 * @typedef {Object} CampfireRecipeResult
 ...
 * @property {'addCampfire'} format Handler format identifier for normalization.
 ...
 * @property {number} cookTime Cooking time in ticks for the campfire recipe.
 */

/**
 * Recipe handler for CraftTweaker campfire cooking recipes.
 *
 * This handler processes recipe segments containing calls to campfire.addRecipe()
 * methods. Campfire recipes define cooking operations with specific inputs,
 * outputs, experience rewards, and cooking times for the campfire block.
 */
module.exports = {
    name: 'campfire-handler',

    /**
     * Evaluates whether this handler can process the given segment.
     *
     * Returns a positive score when the segment contains a campfire.addRecipe call.
     ...
     * @returns {number} Compatibility score: 1 if segment contains campfire.addRecipe, 0 otherwise.
     */
    canParse(segment) {
        if (!segment?.rawText) {
            return 0;
        ...
        return segment.rawText.includes('campfire.addRecipe(') ? 1 : 0;
    },

    /**
     * Extracts recipe data from a campfire.addRecipe method call segment.
     *
     ...
     *
     * @param {Object} segment Recipe segment containing campfire.addRecipe call.
     ...
     * @returns {CampfireRecipeResult} Parsed recipe data ready for normalization.
     * @throws {Error} When the segment does not match the expected campfire.addRecipe pattern.
     */
    parse(segment) {
        const match = segment.rawText.match(/campfire\.addRecipe\("([^"]+)",\s*([^,]+),\s*([^,]+),\s*([^,]+),\s*([^)]+)\);\s*$/);

        if (!match) {
            throw new Error('Unable to match campfire.addRecipe pattern');
        }
